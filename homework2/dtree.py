...
def dtree(data, targets, features, **kwargs):
    ...
    def make_tree(data, weights, classes, featureNames, maxlevel=-1, level=0, forest=0, minData=-1):
        """ The main function, which recursively constructs the tree"""
        nonlocal total_depth, total_leaves
        ...
        if outtype == "classification" and (nData == 0 or nFeatures == 0 or (maxlevel >= 0 and level > maxlevel)):
            ...
            for value in values:
                ...
                if not weighted:
                    newWeights = weights
                subtree = make_tree(
                    np.array(newData), np.array(newWeights), np.array(newClasses), newNames, maxlevel, level+1, forest, minData)
    ...
    def get_cutoff_point(data, weights, classes, feature):
        if outtype == "classification":
            ...
            prev = sort_zipped[0][1]
            boundaries = []

            for idx, tup in enumerate(sort_zipped):
                item = tup[1]
                if item != prev:
            ...
            if len(boundaries) > 8:
                new_boundaries = []
                [new_boundaries.append(np.quantile(boundaries, float(i)/8)) for i in range(1, 8)]
                def get_closest(point, array):
                    i = len(array) - 1
                    while point < array[i] and i > 0:
                        i -= 1
                    if i == len(array)-1:
                        return array[i-1]
                    elif i == 0:
                        return array[i]
                    elif abs(array[i] - point) < abs(array[i+1] - point):
                        return array[i]
                    else:
                        return array[i+1]
                temp = [get_closest(bdry, boundaries) for bdry in new_boundaries]
                boundaries = temp
        else:
    ...
    dTree = make_tree(data, weights, classes, featureNames, maxlevel, level, forest, minData)

    return DTree(dTree, featureNames, total_depth, total_leaves)


class DTree:
    def __init__(self, decision_tree, feature_names, num_depth, num_leaves):
        self.decision_tree = decision_tree
        self.feature_names = feature_names
        self.total_depth = num_depth
        self.total_leaves = num_leaves

    def print_tree(self):
        print(self.decision_tree)

    def predict(self, data):
        return self.classify(self.decision_tree, data)
    ...
    def classify(self, tree, data):
        ...
        ndarrayData = np.array(data)
        if len(ndarrayData.shape) == 1:
            return self.classifySingle(tree, data)
        elif ndarrayData.shape[1] == 1:
            return self.classifySingle(tree, data[:, 0])
        else:
            return self.classifyAll(tree, data)

    def classifySingle(self, tree, datapoint):
        if tree is None:
            return None
        elif tree.isLeaf():
            return tree.key
    ...
    def classifyAll(self, tree, data):
        results = []
        for i in range(data.shape[0]):
            results.append(self.classifySingle(tree, data[i]))
        return results
    ...
    def score(self, data, targets):
        ...
        if np.shape(predictions) != np.shape(targets):
            raise ValueError(f"Shape of predictions {np.shape(predictions)} is not the same as targets {np.shape(targets)}")
